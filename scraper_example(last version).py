...
兩種爬蟲類型，涵蓋 12 個來源的兩種型態：
  ...
  A. RSS 來源 -> fetch_rss_source()
     已實測確認可用：Reddit、投影時代、DigiTimes、TrendForce（Display /
     Consumer Electronics）、IT之家。其中投影時代的 RSS 直接提供完整全文，
     其餘幾個是「綜合性」feed，需搭配 PROJECTOR_KEYWORDS 過濾。

  B. 純 HTML 列表頁、無 RSS -> fetch_html_list_source()
     已實測確認可用：ZOL投影機頻道、ZNDS投影頻道、洛圖科技(RUNTO)、
     ProjectorCentral。文章詳情頁一律改用穩定的 SEO meta 標籤
     （description / og:title / article:published_time）取資料，
     比針對每個網站硬寫正文 CSS class 更耐用。

  尚未整合：
    - AVS Forum：對自動化請求直接回 402（疑似機器人偵測/反爬），
      需要換一套抓取策略（例如瀏覽器自動化），這裡先不列入。
    - ProjectorReviews、SID：尚未驗證，SID 更新頻率低，建議先用人工
      每月複查取代自動化爬蟲。
...
def _fetch_article_meta(url: str, encoding: str) -> dict | None:
    ...
    這個做法比針對每個網站硬寫正文 CSS selector 穩定得多，因為 description /
    og:title 這類 SEO meta 標籤在絕大多數新聞站都存在且格式穩定。"""
    try:
        ...
        title = get_meta("og:title") or (soup.title.get_text(strip=True) if soup.title else "")
        summary = get_meta("description")
        # 不同網站可能用 article:published_time 或 og:updated_time 等不同 key。
        published_raw = get_meta("article:published_time") or get_meta("og:updated_time")
        publish_date = published_raw[:10] if published_raw else time.strftime("%Y-%m-%d")
